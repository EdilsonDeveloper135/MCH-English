...
async def _to_vocabulary_out(db: AsyncSession, item: VocabularyItem) -> VocabularyItemOut:
    translation = await dictionary_service.lookup(db, item.word)
...
async def list_vocabulary(current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    return [await _to_vocabulary_out(db, item) for item in items]
