...
from app.services import achievements_service, gamification_service, recall_service, tts_service
...
async def create_dictation_attempt(
    ...
    # against a sentence_id belonging to a different user's private text or unrelated text.
    if session.text_id is not None:
...
async def finish_dictation_session(
    ...
    await achievements_service.evaluate_and_unlock(db, current_user.id)
